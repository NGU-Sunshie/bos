...
<script type="text/javascript">
	...
	$(function() {
		...
		// 基本功能菜单加载
		$.ajax({
			url : 'functionAction_findMenu.action',
			type : 'POST',
			dataType : 'text',
		...
		$("#btnEp").click(function(){
			...
			if(v){
				//验证通过
				var p1 = $("#txtNewPass").val();//第一次密码
				var p2 = $("#txtRePass").val();//第二次密码
				if(p1 == p2){
					//两次密码一致
					$.post("userAction_editPassword.action",{"password":p1},function(data){
						if(data == "1"){
							//修改密码成功,关闭密码修改窗口
							$("#editPwdWindow").window("close");
						}else{
					...
					//两次密码不一致，弹框提示
					$.messager.alert('提示信息','输入的密码不一致!','warning'); 
				}
			}
		});
	});
	...
	// 版权信息
	function showAbout(){
		$.messager.alert("BOS物流管理系统 v1.0","管理员邮箱: 549917609@qq.com");
	}
</script>
</head>
				...
				<tr>
					<td style="width: 300px;">
						<div style="color: #999; font-size: 8pt;">
							xxx | Powered by <a href="https://www.baidu.com/">xxx.cn</a>
						</div>
					</td>
					...
					<table cellpadding=3>
						<tr>
							...
							<td><input required="required"
								data-options="validType:'length[4,6]'" id="txtNewPass"
								type="Password" class="txt01 easyui-validatebox" /></td>
						</tr>
						<tr>
							<td>确认密码：</td>
							<td><input required="required"
								data-options="validType:'length[4,6]'" id="txtRePass"
								type="Password" class="txt01 easyui-validatebox" /></td>
						</tr>
					</table>
				</form>
